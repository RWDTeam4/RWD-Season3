...
static inline void setSpeed(motor_label motor_label, volatile int speed)
{
	...
	if (motor_label == MOTOR1)
	{
		...
		speed = prev_speed1 + (speed - prev_speed1) * FILTER_CONSTANT;
		prev_speed1 = speed;
	...
	else if (motor_label == MOTOR2)
	{
		...
		pwmPin = motor2PWMpin;
		pthread_mutex_lock(&time_lock);
		speed = prev_speed2 + (speed - prev_speed2) * FILTER_CONSTANT;
		prev_speed2 = speed;
		pthread_mutex_unlock(&time_lock);
	}
	if (speed < 0)
	...
	if (speed != 0 && abs(speed) < MIN_SPEED)
	{
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "leftController");
  ros::NodeHandle n;
  ros::Timer stop_timer = n.createTimer(ros::Duration(0.3), stop_timer_cb);

  ros::Subscriber sub = n.subscribe("Cmd", 1000, cmdCallback);

  message_filters::Subscriber<rwd::Speed> lf_sub(n, "LFSpeed", 10);
  message_filters::Subscriber<rwd::Speed> lr_sub(n, "LRSpeed", 10);
